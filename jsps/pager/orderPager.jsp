...
<script type="text/javascript">
	window.onload = function(){
		var ostate = "${order.ostate}";
		if(ostate!=""){
			var as = document.getElementsByName("a");
			for (var i = 0; i < as.length; i++) {
				var href = as[i].getAttribute("href");
				as[i].setAttribute("href",href+"&ostate="+ostate);
			}
		}
	}
	function _go() {
		...
		var ostate = "${order.ostate}";
		if(ostate!=""){
			location = "${pageContext.request.contextPath}/adminOrder/bkfindOrderByPageBean.action?currPage="+pc+"&ostate="+ostate;
		}else{
			location = "${pageContext.request.contextPath}/adminOrder/bkfindOrderByPageBean.action?currPage="+pc;
		}
  ...
  <div class="divContent">
    <%--上一页 --%>
       ...
       <c:choose>
   			...
   			<c:otherwise>
   				 <a name="a" href="<c:url value='/adminOrder/bkfindOrderByPageBean.action?currPage=${pageBean.currPage-1 }'/>"
   ...
   <c:forEach begin="1" var="page" end="${pageBean.totalPage }" step="1">
   		<c:choose>
   			...
   			<c:otherwise>
   				 <a name="a" href="<c:url value='/adminOrder/bkfindOrderByPageBean.action?currPage=${page }'/>" class="aBtn">${page }</a>
   			</c:otherwise>
   		</c:choose>
   </c:forEach>
