...
<script type="text/javascript">
	...
	function _go() {
		...
		var ostate = "${order.ostate}";
		if(ostate!=""){
			location = "${pageContext.request.contextPath}/order/customerOrderList.action?currPage="+pc+"&ostate="+ostate;
		}else{
			location = "${pageContext.request.contextPath}/order/customerOrderList.action?currPage="+pc;
		}
  ...
  <div class="divContent">
    <%--上一页 --%>
       ...
       <c:choose>
   			...
   			<c:otherwise>
   				 <a name="a" href="<c:url value='/order/customerOrderList.action?currPage=${pageBean.currPage-1 }'/>"
   ...
   <c:forEach begin="1" var="page" end="${pageBean.totalPage }" step="1">
   		<c:choose>
   			...
   			<c:otherwise>
   				 <a name="a" href="<c:url value='/order/customerOrderList.action?currPage=${page }'/>" class="aBtn">${page }</a>
   			</c:otherwise>
   		</c:choose>
   </c:forEach>
